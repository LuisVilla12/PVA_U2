...
class SaltPaperScreen extends StatefulWidget {
  static const String name = 'SaltPaperScreen';
  const SaltPaperScreen({super.key});

  @override
  State<SaltPaperScreen> createState() => _SaltPaperScreenState();
}

class _SaltPaperScreenState extends State<SaltPaperScreen> {
  int ksize = 5;
  @override
  Widget build(BuildContext context) {
    final scaffoldKey = GlobalKey<ScaffoldState>(); 
    // Pantalla para operaciones morfologicas apertura
    return Scaffold(
      appBar: AppBar(title: const Text('Ruido sal y pimienta')),
      body: SafeArea(
        child: Padding(
                  ...
                  ImageSelectorWidget(
                    onImageSelected: (path) async {
                      ...
                      try {
                        ...
                        final Uint8List? bytes = await UploadService.uploadImage('/salt_pepper', path, ksize: ksize);
                        ...
                        showDialog(
                          ...
                          builder: (_) => AlertDialog(
                            title: Text('Preview sal y pimienta (ksize=$ksize)'),
                            ...
                            actions: [
                              IconButton(
                                ...
                                onPressed: () async {
                                  try {
                                    await UploadService.saveImageBytes(bytes, 'sal_pimienta.png');
