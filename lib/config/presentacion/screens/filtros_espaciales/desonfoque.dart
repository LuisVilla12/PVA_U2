...
class DesonfoqueScreen extends StatefulWidget {
  ...
  const DesonfoqueScreen({super.key});

  @override
  State<DesonfoqueScreen> createState() => _DesonfoqueScreenState();
}

class _DesonfoqueScreenState extends State<DesonfoqueScreen> {
  int ksize = 5;
  
  @override
  Widget build(BuildContext context) {
    final scaffoldKey = GlobalKey<ScaffoldState>(); 
    // Pantalla para el filtro espacial desenfoque
    return Scaffold(
      appBar: AppBar(
                  ...
                  ImageSelectorWidget(
                    onImageSelected: (path) async {
                      ...
                      try {
                        ...
                        final Uint8List? bytes = await UploadService.uploadImage('/desenfoque', path, ksize: ksize);
                        ...
                        showDialog(
                          ...
                          builder: (_) => AlertDialog(
                            title: Text('Preview Desenfoque (ksize=$ksize)'),
                            ...
                            actions: [
                              IconButton(
                                ...
                                onPressed: () async {
                                  try {
                                    await UploadService.saveImageBytes(bytes, 'desenfoque.png');
